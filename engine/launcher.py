...
from engine.utils import get_crawl_args, native_stringify_dict
...
class Launcher(Service):
    ...
    def __init__(self, config, app):
        ...
        self.runner = config.get('runner', 'engine.runner')
        self.app = app

    def startService(self):
        for slot in range(self.max_proc):
            self._wait_for_project(slot)
        log.msg(format='DtCrawlEngine %(version)s started: max_proc=%(max_proc)r, runner=%(runner)r',
