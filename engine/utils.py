...
from engine.spiderqueue import SqliteSpiderQueue
...
class JsonResource(resource.Resource):
    ...
    def render(self, tx_request):
        r = resource.Resource.render(self, tx_request)
        return self.render_object(r, tx_request)

    def render_object(self, obj, tx_request):
        r = self.json_encoder.encode(obj) + "\n"
        tx_request.setHeader('Content-Type', 'application/json')
        tx_request.setHeader('Access-Control-Allow-Origin', '*')
        tx_request.setHeader('Access-Control-Allow-Methods', 'GET, POST, PATCH, PUT, DELETE')
        tx_request.setHeader('Access-Control-Allow-Headers', ' X-Requested-With')
        tx_request.setHeader('Content-Length', len(r))
        return r
...
def get_project_list(config):
    ...
    the engine.conf [settings] section
    """
